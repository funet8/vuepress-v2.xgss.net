import{_ as n}from"./plugin-vue_export-helper-DlAUqK2U.js";import{c as a,b as i,o as e}from"./app-DbDHfX93.js";const l={};function p(d,s){return e(),a("div",null,s[0]||(s[0]=[i(`<h1 id="nginx从入门到放弃03-nginx调优" tabindex="-1"><a class="header-anchor" href="#nginx从入门到放弃03-nginx调优"><span>Nginx从入门到放弃03-Nginx调优</span></a></h1><h2 id="一、调优的必要性" tabindex="-1"><a class="header-anchor" href="#一、调优的必要性"><span>一、调优的必要性</span></a></h2><p>在聊调优之前，我们先要知道为何调优，业务运行和调优的关系。</p><p>笔者把自己总结的文档分为几遍，合集在 https://g.xgss.net/nginx/</p><p><img src="https://imgoss.xgss.net/picgo/1607251234038.png?aliyun" alt="42967083431970b0eb40b3949278a0d1.png"></p><p>业务运行：线上业务正常运行，承载了公司业务。 监控业务：通过监控业务对线上业务进行监控，及时发现问题。 优化业务：通过监控分析，发现业务问题或者瓶颈，及时对业务或者软件就行调整、优化。 测试优化：优化完成后，需要对现有的优化进行测试，保证业务在当前优化模式中稳定、高效，能够解决当前问题。 这就是业务运行的一个流程，也是我们保证业务稳定、高效、高可用的运维之道。</p><h2 id="二、调优" tabindex="-1"><a class="header-anchor" href="#二、调优"><span>二、调优</span></a></h2><p>调优类的文章是最难写的，因为我只能告诉你调优的选项，无法告诉你具体的阈值，因为不同的业务运行在不同的机器，所消耗的资源是不同的；又因为场景不同，对应的调优项及阈值是千变万化的。不能为了调优而调优，要根据实际情况、测试环境还是生产环境、实际业务等需求来实际配置，所以nginx的基本配置需要了解是什么意思，才能调优</p><h3 id="cpu优化" tabindex="-1"><a class="header-anchor" href="#cpu优化"><span>CPU优化</span></a></h3><p>1）为什么要绑定nginx进程到不同的CPU上：CPU调度的时候两个进程有可能被分配达到一个CPU上，从而会导致一个非常的空闲，一个非常的忙，无法充分发挥CPU的运算能力</p><p>（2）如何分配不同的nginx进程给不同的CPU处理</p><div class="language- line-numbers-mode" data-highlighter="shiki" data-ext="" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34;"><pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code class="language-"><span class="line"><span>4核CPU</span></span>
